import { Gender } from "@prisma/client";
import type { LoaderFunctionArgs } from "@remix-run/node";
import { getGenders } from "~/models/gender.server";
import { getPersonList } from "~/models/person.server";
import { requireUserId } from "~/session.server";
import type { SimplePersonValidated } from "~/validators/person";
import { simplePersonSchema } from "~/validators/person";

export interface EntityLoaderReturnType {
  genders: { value: Gender; label: string }[];
  persons: SimplePersonValidated[];
  mothers: SimplePersonValidated[];
  fathers: SimplePersonValidated[];
}

export async function entitiesLoader(
  request: LoaderFunctionArgs["request"]
): Promise<EntityLoaderReturnType> {
  const userId = await requireUserId(request);

  const genders = getGenders();
  const persons = await getPersonList({ id: userId });
  const mothers = persons.filter(({ gender }) => gender === Gender.FEMALE);
  const fathers = persons.filter(({ gender }) => gender === Gender.MALE);

  return {
    genders,
    persons: persons.map((person) => simplePersonSchema.parse(person)),
    mothers,
    fathers,
  };
}
